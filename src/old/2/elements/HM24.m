classdef HM24
    ...
    methods
        function obj = HM24(material)
            tmp = 1/600;
            obj.density = material.density * (tmp(ones(24,24)) + diag(tmp(ones(1,24))));
            obj.viscosity = material.viscosity;
            obj.elasticity = zeros(18, 18);
            obj.elasticity(1:3,1:3) = material.firstLame * ones(3);
            obj.elasticity(1:3,1:3) += 2 * material.secondLame * diag(ones(1,3),3,3);
            obj.elasticity(4:end,4:end) = diag(material.secondLame * ones(15,1));
            obj.param = 1.0;
        end
%
...
%             n = rows(grid.elem);
%             m = 3 * rows(grid.mesh);
%             ij_trip = repelem(3*grid.elem.'(:),3) + repmat(int32(-2:0),1,n*8)';
%             i_trip = repelem(reshape(ij_trip,24,[]),1,24)(:);
%             j_trip = repelem(ij_trip,24);
%             switch nargout
%                 case 1
%                     stiffness = arrayfun(@(i) obj.compute(i, grid),1:n,'UniformOutput',false);
        ...
        function [K,M,D] = compute(obj,points)
            ...
            if isargout(D)
                if isargout(K)
                    ...
                    D = -obj.viscosity * B' * obj.elasticity * B * volume;
                end
            end
        end
